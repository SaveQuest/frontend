...
export const BASEURL = {
  development: 'http://10.246.236.195:3000',
  production: 'https://sq-production-v0.mintpasta.store'
}
...
class APIRequester {
  ...
  /**
   * 챌린지 페이지 헤더 정보를 가져옵니다.
   * @returns {Promise<Record<string, Any>>}
   * @example
   * const response = await apiRequester.fetchDSTChallengeHeader();
   * console.log(response.data);
   * @data https://jsoneditoronline.org/#left=cloud.8b966208a5c1451e8fc12a86b0b4114c
   */
  async fetchDSTChallengeHeader() {
    const response = await this.session.get('/challenge/dst/header');
    return response.data;
  }
...
export const requester = new APIRequester('production', false);
